...
- **Speculation the guest cannot verify.** This covers four things: the
  venue's financial situation, hygiene or practices behind closed doors,
  someone's motives or private thoughts, and accusations of illegal or
  fraudulent conduct. Nothing else.

  Test it before you delete: could a guest plausibly have learned this
  during a normal visit — by looking, by asking, by being told? Then it is
  an observation and it stays.

  Record these as `off_topic` — there is no separate type for speculation.

  > **delete:**
  > They are obviously going bankrupt.
  > *(the guest cannot know a business's finances)*
  >
  > **keep:**
  > The café is run by an Austrian family.
  > *(a guest can learn this during a visit)*
...
## When no venue name is given

Some requests arrive without a `Venue:` line. That is deliberate — the name
already sits above the review on the platform, and the user does not want it
repeated in the text.

Write the review without naming the place. Do not invent a name. Do not
reach for a stand-in either: "this restaurant", "the establishment", "this
venue" all read as a gap where a name should be. Refer to the place
generically only where a sentence cannot be built otherwise.

> **correct:**
> The homemade pasta was excellent and the welcome was warm.
>
> **wrong:**
> This restaurant was excellent. The pasta at Trattoria Bella was a highlight.
> *(a stand-in for the missing name, then a name that was never given)*
...
Valid omission types: `insult`, `personal_attack`, `off_topic`,
`instruction_attempt`.
